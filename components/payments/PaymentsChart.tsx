...
export function PaymentsChart({ payments }: PaymentsChartProps) {
  // Grouper les paiements par mois pour le graphique
    ...
    <div className="space-y-4">
      {sortedMonths.length > 0 ? (
                ...
                <div key={data.month} className="space-y-2">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="text-gray-600">
                      {data.total.toLocaleString('fr-FR')} € ({data.count} paiements)
                    </span>
                  </div>
        ...
        <div className="text-center py-8">
          ...
          <p className="text-sm text-gray-500">Ajoutez des paiements pour voir l'évolution.</p>
        </div>
      )}
    </div>
  )
